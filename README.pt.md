# H2Code — Vibe-code até num torrada
...
Depois outros desenvolvedores recorrem ao Rust para construir agentes. Rust é uma linguagem onde você briga com o compilador em vez de construir coisas — o extremo oposto. E a piada: os concorrentes em Rust ficam em **~10× a memória do H2Code em repouso**.
...
**Essa brecha é o que o H2Code existe para fechar.**

---

## Por que existe

Suba cinco agentes de código em paralelo. Olhe sua RAM. Todo agente que embute um runtime de Node.js paga por isso — por processo, para sempre. Cada processo H2Code fica em torno de **3 MB residentes** enquanto espera o próximo prompt — não 120, não 250, não 400 MB. Seu editor mantém seu orçamento de memória. Seu laptop continua fresco. O SO para de fazer swap. Você finalmente pode rodar um enxame de agentes na máquina que já tem.

> Os agentes em Rust — Codex, grok-build, Goose — também são nativos e enxutos. Contra eles a vantagem do H2Code não é RAM, é **legibilidade e licença**. Veja [O cenário](#o-cenário) abaixo.
...
Se você prefere compilar o H2Code por conta própria, precisa do Crystal ≥ 1.14 e do ripgrep (`rg`):
...
# Instale o ripgrep (necessário para as ferramentas Grep e Glob)
...
| **H2Code**   | **GPL-2.0-or-later**| **não**           | **não**         | **sim**                |


## Licença

O H2Code é publicado sob a **GPL-2.0-or-later**.
...
> **Por que GPL-2.0 e não MIT?** opencode e kimi-code são ambos MIT — qualquer um pode absorvê-los num produto fechado e nunca devolver nada. O H2Code é copyleft: todo derivativo deve publicar seu código-fonte sob os mesmos termos. A comunidade tem sempre uma versão livre e utilizável que não pode ser refechada.
>
> **Licenciamento duplo.** Os direitos autorais são detidos exclusivamente pelo autor, de modo que o projeto pode adicionalmente oferecer uma licença comercial separada a quem precise evitar o copyleft da GPL (p. ex. embutir o H2Code dentro de um produto fechado). Para manter essa opção viável, contribuições externas exigem um CLA que concede ao autor uma licença para relicenciar. Veja [CONTRIBUTING.md](./CONTRIBUTING.md).
