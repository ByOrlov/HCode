...
describe Hcode::Loop::RetryPolicy do
  it "computes exponential backoff capped at max_delay" do
    policy = Hcode::Loop::RetryPolicy.new(max_retries: 4, base_delay: 2, max_delay: 30)
  ...
  it "caps delay at max_delay" do
    policy = Hcode::Loop::RetryPolicy.new(max_retries: 6, base_delay: 2, max_delay: 30)
  ...
  it "returns nil when retries are exhausted" do
    policy = Hcode::Loop::RetryPolicy.new(max_retries: 3)
  ...
  it "considers ApiError retryable only when its flag is set" do
    policy = Hcode::Loop::RetryPolicy.new

    retryable = Hcode::LLM::ApiError.new(429, "rate limited", retryable: true)
    fatal = Hcode::LLM::ApiError.new(401, "unauthorized", retryable: false)
  ...
  it "never retries user cancellation" do
    policy = Hcode::Loop::RetryPolicy.new
    policy.retryable?(Hcode::Loop::UserCancellationError.new("cancelled")).should be_false
  end
end
