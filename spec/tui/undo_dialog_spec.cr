...
private def make_choice(count : Int32, input : String, label : String)
  Hcode::TUI::UndoDialog::Choice.new("c#{count}", count, input, label)
end
...
private def noop_select
  Proc(Hcode::TUI::UndoDialog::Choice, Nil).new { |_| }
end

describe Hcode::TUI::UndoDialog do
  it "opens hidden and becomes visible after show" do
    dialog = Hcode::TUI::UndoDialog.new(Hcode::TUI::Theme.dark)
  ...
  it "selects the current choice on Enter and invokes callback" do
    dialog = Hcode::TUI::UndoDialog.new(Hcode::TUI::Theme.dark)
    selected = [] of Int32
    handler = Proc(Hcode::TUI::UndoDialog::Choice, Nil).new do |c|
      selected << c.count
    end
  ...
  it "navigates up/down with arrow keys" do
    ...
    dialog.handle_input(key_event(Hcode::TUI::Key::Up))
  ...
  it "renders empty state when no choices" do
    dialog = Hcode::TUI::UndoDialog.new(Hcode::TUI::Theme.dark)
    empty_arr = [] of Hcode::TUI::UndoDialog::Choice
