...
describe Kimi::LLM::Message do
  describe ".user" do
    it "creates a user message" do
      msg = Kimi::LLM::Message.user("hello")
  ...
  describe ".assistant" do
    it "creates an assistant message with text only" do
      msg = Kimi::LLM::Message.assistant("hi there")
    ...
    it "creates an assistant message with tool_calls" do
      tc = Kimi::LLM::ToolCall.new("call_1", Kimi::LLM::ToolCallFunction.new("Bash", "{\"command\":\"ls\"}"))
      msg = Kimi::LLM::Message.assistant("", [tc])
      msg.role.should eq("assistant")
  ...
  describe ".tool" do
    it "creates a tool result message" do
      msg = Kimi::LLM::Message.tool("result text", "call_1")
  ...
  describe "JSON serialization" do
    it "omits nil fields" do
      msg = Kimi::LLM::Message.user("test")
    ...
    it "includes tool_calls when present" do
      tc = Kimi::LLM::ToolCall.new("call_1", Kimi::LLM::ToolCallFunction.new("Bash", "{}"))
      msg = Kimi::LLM::Message.assistant(nil, [tc])
      json = JSON.parse(msg.to_json)
...
describe Kimi::LLM::Usage do
  it "adds two usages" do
    u1 = Kimi::LLM::Usage.new(prompt_tokens: 100, completion_tokens: 50, total_tokens: 150)
    u2 = Kimi::LLM::Usage.new(prompt_tokens: 200, completion_tokens: 100, total_tokens: 300)
    combined = u1 + u2
...
describe Kimi::LLM::StreamChunk do
  it "parses usage from the top-level field" do
    chunk = Kimi::LLM::StreamChunk.from_json(%({"usage":{"prompt_tokens":42,"completion_tokens":7,"total_tokens":49}}))
  ...
  it "parses Moonshot-proprietary usage from choices[0].usage" do
    chunk = Kimi::LLM::StreamChunk.from_json(%({"choices":[{"index":0,"usage":{"prompt_tokens":120,"completion_tokens":30,"total_tokens":150}}]}))
...
describe Kimi::LLM::ChatRequest do
  it "emits only the base fields when nothing extra is configured" do
    req = Kimi::LLM::ChatRequest.new("m", [Kimi::LLM::Message.user("hi")])
  ...
  it "includes prompt_cache_key and stream_options when streaming" do
    req = Kimi::LLM::ChatRequest.new("m", [Kimi::LLM::Message.user("hi")], stream: true)
  ...
  it "prefers max_completion_tokens over the legacy max_tokens alias" do
    req = Kimi::LLM::ChatRequest.new("m", [Kimi::LLM::Message.user("hi")], max_tokens: 4096)
  ...
  describe "thinking wire dialects" do
    it "Kimi dialect emits a thinking object without effort for boolean-only models" do
      req = Kimi::LLM::ChatRequest.new("m", [Kimi::LLM::Message.user("hi")])
      req.thinking = Kimi::LLM::ThinkingConfig.new("enabled")
      json = JSON.parse(req.to_json)
  ...
  it "includes parallel_tool_calls when set" do
    tool = Kimi::LLM::ToolDefinition.new(Kimi::LLM::ToolFunction.new("Read", "reads", JSON.parse("{}")))
    req = Kimi::LLM::ChatRequest.new("m", [Kimi::LLM::Message.user("hi")], tools: [tool])
...
describe Kimi::LLM::ApiError do
  describe ".retryable_status?" do
    it "treats rate-limit and request-timeout as retryable" do
      Kimi::LLM::ApiError.retryable_status?(408).should be_true
      Kimi::LLM::ApiError.retryable_status?(429).should be_true
    end

    it "treats 5xx server errors as retryable" do
      Kimi::LLM::ApiError.retryable_status?(500).should be_true
      Kimi::LLM::ApiError.retryable_status?(502).should be_true
      Kimi::LLM::ApiError.retryable_status?(503).should be_true
      Kimi::LLM::ApiError.retryable_status?(504).should be_true
    end

    it "treats auth/quota/not-found/bad-request as non-retryable" do
      {400, 401, 403, 404, 422}.each do |code|
        Kimi::LLM::ApiError.retryable_status?(code).should be_false
      end
    end
  end

  it "carries status code and the computed retryable flag" do
    err = Kimi::LLM::ApiError.new(403, "quota exceeded", retryable: false)
...
describe Kimi::LLM::Provider do
  it "is the base class of KimiProvider" do
    provider = Kimi::LLM::KimiProvider.new(model: "kimi-for-coding")
    provider.is_a?(Kimi::LLM::Provider).should be_true
  end
end

describe Kimi::LLM::KimiProvider do
  it "identifies itself as the kimi backend" do
    provider = Kimi::LLM::KimiProvider.new(model: "kimi-for-coding")
    provider.name.should eq("kimi")
  end

  it "exposes the upstream model name" do
    provider = Kimi::LLM::KimiProvider.new(model: "kimi-for-coding")
    provider.model_name.should eq("kimi-for-coding")
  end

  it "shares the OpenAI Chat Completions transport" do
    provider = Kimi::LLM::KimiProvider.new(model: "kimi-for-coding")
    provider.is_a?(Kimi::LLM::OpenAIChatProvider).should be_true
  end
end

describe Kimi::LLM::ZaiProvider do
  it "is a Provider sharing the OpenAI Chat Completions transport" do
    provider = Kimi::LLM::ZaiProvider.new(api_key: "sk-test")
    provider.is_a?(Kimi::LLM::Provider).should be_true
    provider.is_a?(Kimi::LLM::OpenAIChatProvider).should be_true
  end
  ...
  it "honours an explicit model and endpoint override" do
    provider = Kimi::LLM::ZaiProvider.new(model: "glm-4.7", endpoint: "https://custom.example/v4", api_key: "k")
    provider.model_name.should eq("glm-4.7")
    provider.endpoint.should eq("https://custom.example/v4")
  end
end

describe Kimi::LLM::MockProvider do
  ...
  it "replays the script step by step and terminates" do
    provider = Kimi::LLM::MockProvider.new([
      Kimi::LLM::MockStep.new(
        parts: [Kimi::LLM::ToolCallPart.new("c1", "Bash", %({"command":"echo hi"}))] of Kimi::LLM::MessagePart,
        stop_reason: "tool_use",
      ),
      Kimi::LLM::MockStep.new(
        parts: [Kimi::LLM::TextPart.new("done")] of Kimi::LLM::MessagePart,
        stop_reason: "end_turn",
        text: "done",
      ),
    ])

    first = provider.chat([] of Kimi::LLM::Message, nil) { }
    first.tool_use?.should be_true
    first.tool_calls.size.should eq(1)

    second = provider.chat([] of Kimi::LLM::Message, nil) { }
  ...
  it "streams the step's parts through the block before returning" do
    provider = Kimi::LLM::MockProvider.new([
      Kimi::LLM::MockStep.new(
        parts: [
          Kimi::LLM::ToolCallPart.new("c1", "Glob", %({"pattern":"*"})),
          Kimi::LLM::TextPart.new("hello"),
    ...
    seen = [] of Kimi::LLM::MessagePart
    provider.chat([] of Kimi::LLM::Message, nil) { |part| seen << part }

    seen.any?(&.is_a?(Kimi::LLM::ToolCallPart)).should be_true
    seen.any?(&.is_a?(Kimi::LLM::TextPart)).should be_true
    seen.last.is_a?(Kimi::LLM::FinishPart).should be_true
  end
end

describe "LLM provider registry" do
  it "defaults to the kimi provider" do
    Kimi::LLM::DEFAULT_PROVIDER_NAME.should eq("kimi")
  end

  it "lists kimi, zai and mock among the known providers" do
    names = Kimi::LLM::KNOWN_PROVIDERS.map(&.name)
    names.should contain("kimi")
    names.should contain("zai")
    names.should contain("mock")
  end

  it "recognises known providers and rejects unknown ones" do
    Kimi::LLM.known_provider?("kimi").should be_true
    Kimi::LLM.known_provider?("zai").should be_true
    Kimi::LLM.known_provider?("mock").should be_true
    Kimi::LLM.known_provider?("nope").should be_false
  end
end
...
describe "LLM model registry" do
  ...
  it "fetch_models respects a custom model name on MockProvider" do
    provider = Kimi::LLM::MockProvider.new(model: "custom-mock")
    provider.fetch_models.should eq(["custom-mock"])
  end
end

describe "OpenAIChatProvider request shaping" do
  it "KimiProvider sends prompt_cache_key, thinking and max_completion_tokens" do
    provider = Kimi::LLM::KimiProvider.new(model: "kimi-for-coding")
    ...
    json = JSON.parse(provider.build_request([Kimi::LLM::Message.user("hi")], nil).to_json)
  ...
  it "KimiProvider omits effort for boolean-only models (kimi-for-coding has no valid_efforts)" do
    provider = Kimi::LLM::KimiProvider.new(model: "kimi-for-coding")
    provider.thinking_effort = "medium"
  ...
  it "KimiProvider sends effort when the model declares it in valid_efforts" do
    provider = Kimi::LLM::KimiProvider.new(model: "k3")
  ...
  it "KimiProvider falls back to default_effort when the requested one is unsupported" do
    provider = Kimi::LLM::KimiProvider.new(model: "k3")
  ...
  it "KimiProvider disables thinking for the off token" do
    provider = Kimi::LLM::KimiProvider.new(model: "kimi-for-coding")
  ...
  it "enables parallel_tool_calls when tools are present" do
    provider = Kimi::LLM::KimiProvider.new(model: "kimi-for-coding")
    tool = Kimi::LLM::ToolDefinition.new(Kimi::LLM::ToolFunction.new("Read", "reads", JSON.parse("{}")))
    json = JSON.parse(provider.build_request([Kimi::LLM::Message.user("hi")], [tool]).to_json)
    json["parallel_tool_calls"].should be_true
  end
