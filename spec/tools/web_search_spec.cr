...
private class FakeProvider < H2code::Tools::WebSearchProvider
  def initialize(@behaviour : Proc(String, Array(H2code::Tools::WebSearchResult)))
  end

  def search(query : String, tool_call_id : String? = nil, signal : H2code::Tools::AbortController? = nil) : Array(H2code::Tools::WebSearchResult)
    @behaviour.call(query)
  end
end

private class FakeService < H2code::Tools::WebSearchProviderService
  def initialize(@provider : H2code::Tools::WebSearchProvider?)
  end

  def get_web_search_provider : H2code::Tools::WebSearchProvider?
    @provider
  end
end

describe H2code::Tools::WebSearch do
  after_each do
    H2code::Tools::WebSearch.service = nil
  end

  it "exposes JS-name and identical schema" do
    tool = H2code::Tools::WebSearch.new
    tool.name.should eq(H2code::Tools::Names::WEB_SEARCH)
  ...
  it "returns NO_RESULTS_MESSAGE on empty list" do
    H2code::Tools::WebSearch.service = FakeService.new(FakeProvider.new(->(_q : String) { [] of H2code::Tools::WebSearchResult }))
  ...
  it "renders list with site/date when present" do
    H2code::Tools::WebSearch.service = FakeService.new(FakeProvider.new(->(_q : String) do
      [
        H2code::Tools::WebSearchResult.new(title: "T1", url: "https://a.com", snippet: "S1",
          site_name: "SiteA", date: "2026-01-01"),
        H2code::Tools::WebSearchResult.new(title: "T2", url: "https://b.com", snippet: "S2"),
      ]
    end))
  ...
  it "returns error when no provider is configured" do
    H2code::Tools::WebSearch.service = FakeService.new(nil)
  ...
  it "truncates very large result sets" do
    H2code::Tools::WebSearch.service = FakeService.new(FakeProvider.new(->(_q : String) do
      (1..200).map do |i|
        H2code::Tools::WebSearchResult.new(title: "T#{i}", url: "https://#{i}.com", snippet: "S" * 300)
      end.to_a
    end))
    ...
    result = tool.execute(JSON.parse(%({ "query": "x" })))
    result.is_error?.should be_false
    result.content.size.should be < (H2code::Tools::WebSearch::MAX_CHARS + 200)
    result.content.should contain(H2code::Tools::WebSearch::TRUNCATION_MARKER)
    result.truncated?.should be_true
  end
end

describe H2code::Tools::MoonshotWebSearchProvider do
  it "parses results with all fields" do
    ...
    provider = H2code::Tools::MoonshotWebSearchProvider.new("https://example.com/search", "key")
  ...
  it "uses injected transport and parses results" do
    ...
    provider = H2code::Tools::MoonshotWebSearchProvider.new(
...
describe H2code::Tools::ProviderWebSearchService do
  it "returns nil for zai provider" do
    llm = H2code::LLM::ZaiProvider.new(
      model: "glm-4.6",
      endpoint: "https://api.z.ai/api/paas/v4",
      api_key: "sk-test",
      provider_label: "zai")
    service = H2code::Tools::ProviderWebSearchService.new(llm)
  ...
  it "returns nil for providers without native search" do
    llm = H2code::LLM::MockProvider.new
    service = H2code::Tools::ProviderWebSearchService.new(llm)
    service.get_web_search_provider.should be_nil
  end

  it "builds a Moonshot search provider from the active provider" do
    llm = H2code::LLM::MoonshotProvider.new(
      model: "kimi-for-coding",
      endpoint: "https://api.kimi.com/coding/v1",
      api_key: "sk-test")
    service = H2code::Tools::ProviderWebSearchService.new(llm)
    search = service.get_web_search_provider
    search.should be_a(H2code::Tools::MoonshotWebSearchProvider)
  end
...
describe H2code::Tools::CompositeWebSearchService do
  it "prefers the config service over the provider service" do
    config_provider = FakeProvider.new(->(_q : String) { [] of H2code::Tools::WebSearchResult })
    config_service = FakeService.new(config_provider)
    provider_service = FakeService.new(nil)

    composite = H2code::Tools::CompositeWebSearchService.new(config_service, provider_service)
  ...
  it "falls back to the provider service when config returns nil" do
    ...
    provider_provider = FakeProvider.new(->(_q : String) { [] of H2code::Tools::WebSearchResult })
