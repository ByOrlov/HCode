...
describe "H2code::Tools::Cron.parse_expression" do
  it "parses wildcards" do
    parsed = H2code::Tools::Cron.parse_expression("* * * * *")
  ...
  it "parses */N step" do
    parsed = H2code::Tools::Cron.parse_expression("*/5 * * * *")
    parsed.minute.to_a.sort.should eq([0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55])
  end

  it "parses range" do
    parsed = H2code::Tools::Cron.parse_expression("0 9 * * 1-5")
    parsed.dow.to_a.sort.should eq([1, 2, 3, 4, 5])
  end

  it "parses list" do
    parsed = H2code::Tools::Cron.parse_expression("0,30 * * * *")
    parsed.minute.to_a.sort.should eq([0, 30])
  end

  it "parses stepped range" do
    parsed = H2code::Tools::Cron.parse_expression("0-59/15 * * * *")
    parsed.minute.to_a.sort.should eq([0, 15, 30, 45])
  end

  it "normalizes dow 7 → 0 (Sunday)" do
    parsed = H2code::Tools::Cron.parse_expression("* * * * 7")
    parsed.dow.to_a.sort.should eq([0])
  end

  it "rejects wrong field count" do
    expect_raises(H2code::Tools::CronParseError, /5 fields/) do
      H2code::Tools::Cron.parse_expression("* * *")
    end
  end

  it "rejects out-of-range value" do
    expect_raises(H2code::Tools::CronParseError, /out of range/) do
      H2code::Tools::Cron.parse_expression("60 * * * *")
    end
  end

  it "rejects malformed step" do
    expect_raises(H2code::Tools::CronParseError) do
      H2code::Tools::Cron.parse_expression("abc * * * *")
    end
  end

  it "rejects inverted range" do
    expect_raises(H2code::Tools::CronParseError, /start > end/) do
      H2code::Tools::Cron.parse_expression("10-5 * * * *")
    end
  end
end

describe "H2code::Tools::Cron.to_human" do
  it "every-N-minutes pattern" do
    parsed = H2code::Tools::Cron.parse_expression("*/5 * * * *")
    H2code::Tools::Cron.to_human(parsed).should eq("every 5 minutes")
  end

  it "at-N daily pattern" do
    parsed = H2code::Tools::Cron.parse_expression("0 9 * * *")
    H2code::Tools::Cron.to_human(parsed).should eq("at 9:00 daily")
  end

  it "weekdays pattern" do
    parsed = H2code::Tools::Cron.parse_expression("0 9 * * 1-5")
    H2code::Tools::Cron.to_human(parsed).should eq("at 9:00 on weekdays")
  end

  it "sunday pattern" do
    parsed = H2code::Tools::Cron.parse_expression("0 9 * * 0")
    H2code::Tools::Cron.to_human(parsed).should eq("at 9:00 on Sunday")
  end
end

describe "H2code::Tools::Cron.compute_next_cron_run" do
  it "finds the next minute match for */5" do
    parsed = H2code::Tools::Cron.parse_expression("*/5 * * * *")
    # Fixed reference in local time.
    now = Time.local(2026, 1, 1, 12, 3, 0).to_unix_ms
    nxt = H2code::Tools::Cron.compute_next_cron_run(parsed, now)
...
describe "H2code::Tools::Cron.has_fire_within_years?" do
  it "returns true for valid daily cron" do
    parsed = H2code::Tools::Cron.parse_expression("0 9 * * *")
    now = Time.utc(2026, 1, 1).to_unix_ms
    H2code::Tools::Cron.has_fire_within_years?(parsed, 5, now).should be_true
  end

  it "returns false for Feb 30 (never fires)" do
    parsed = H2code::Tools::Cron.parse_expression("0 0 30 2 *")
    now = Time.utc(2026, 1, 1).to_unix_ms
    H2code::Tools::Cron.has_fire_within_years?(parsed, 5, now).should be_false
  end
end

describe H2code::Tools::CronCreate do
  before_each do
    H2code::Tools::Cron.service = H2code::Tools::InMemoryCronService.new
  end
  after_each do
    H2code::Tools::Cron.service = nil
  end

  it "exposes JS-name and identical schema" do
    tool = H2code::Tools::CronCreate.new
    tool.name.should eq(H2code::Tools::Names::CRON_CREATE)
  ...
  it "rejects prompt > 8 KiB" do
    tool = H2code::Tools::CronCreate.new
    long_prompt = "x" * (H2code::Tools::Cron::MAX_PROMPT_BYTES + 1)
    result = tool.execute(JSON.parse(%({
      "cron": "*/5 * * * *",
      "prompt": "#{long_prompt}"
    })))
    result.is_error?.should be_true
    result.content.should contain("exceeds #{H2code::Tools::Cron::MAX_PROMPT_BYTES} bytes")
  end

  it "rejects when session cap reached" do
    service = H2code::Tools::Cron.service || raise "Cron.service not initialized"
    H2code::Tools::Cron::MAX_CRON_JOBS_PER_SESSION.times do
      service.add_task(H2code::Tools::CronTaskInit.new(cron: "*/5 * * * *", prompt: "x"))
    end
    tool = H2code::Tools::CronCreate.new
    result = tool.execute(JSON.parse(%({
      "cron": "*/5 * * * *",
  ...
  it "honors killswitch (disabled)" do
    service = H2code::Tools::Cron.service.as(H2code::Tools::InMemoryCronService)
    service.disable!
    tool = H2code::Tools::CronCreate.new
    result = tool.execute(JSON.parse(%({
      "cron": "*/5 * * * *",
...
describe H2code::Tools::CronList do
  before_each do
    H2code::Tools::Cron.service = H2code::Tools::InMemoryCronService.new
  end
  after_each do
  ...
  it "exposes JS-name and identical schema" do
    tool = H2code::Tools::CronList.new
    tool.name.should eq(H2code::Tools::Names::CRON_LIST)
    tool.parameters["additionalProperties"].as_bool.should be_false
  end
  ...
  it "lists scheduled jobs with key fields" do
    service = H2code::Tools::Cron.service.as(H2code::Tools::InMemoryCronService)
    task = service.add_task(H2code::Tools::CronTaskInit.new(cron: "*/5 * * * *", prompt: "check the deploy"))
  ...
  it "records separated by ---" do
    service = H2code::Tools::Cron.service || raise "Cron.service not initialized"
    service.add_task(H2code::Tools::CronTaskInit.new(cron: "*/5 * * * *", prompt: "a"))
    service.add_task(H2code::Tools::CronTaskInit.new(cron: "0 9 * * *", prompt: "b"))
    tool = H2code::Tools::CronList.new
    result = tool.execute(JSON.parse(%({})))
  ...
  it "truncates prompt preview > 200 bytes (UTF-8 safe)" do
    service = H2code::Tools::Cron.service || raise "Cron.service not initialized"
    long_prompt = "x" * 250
    service.add_task(H2code::Tools::CronTaskInit.new(cron: "*/5 * * * *", prompt: long_prompt))
    tool = H2code::Tools::CronList.new
    result = tool.execute(JSON.parse(%({})))
  ...
  it "renders defensively on malformed cron stored task" do
    ...
    task = H2code::Tools::CronTask.new(
      id: "deadbeef",
...
describe H2code::Tools::CronDelete do
  before_each do
    H2code::Tools::Cron.service = H2code::Tools::InMemoryCronService.new
  end
  after_each do
  ...
  it "exposes JS-name and identical schema" do
    tool = H2code::Tools::CronDelete.new
    tool.name.should eq(H2code::Tools::Names::CRON_DELETE)
  ...
  it "deletes an existing job" do
    service = H2code::Tools::Cron.service || raise "Cron.service not initialized"
    task = service.add_task(H2code::Tools::CronTaskInit.new(cron: "*/5 * * * *", prompt: "x"))
    tool = H2code::Tools::CronDelete.new
...
describe "H2code::Tools::LiveCronService" do
  ...
  it "does not fire when disabled" do
    svc = H2code::Tools::LiveCronService.new(enabled: false)
    fired = [] of String
    svc.delivery = ->(xml : String) { fired << xml; nil }
  ...
  it "persists and reloads tasks through the store" do
    Dir.tempdir.tap do |tmp|
      store = H2code::Session::Store.new(File.join(tmp, "cron-test-#{Random::Secure.hex(4)}"))
      svc = H2code::Tools::LiveCronService.new(store: store, enabled: true)
      task = svc.add_task(H2code::Tools::CronTaskInit.new(cron: "*/5 * * * *", prompt: "persist me"))
      svc.persist!

      svc2 = H2code::Tools::LiveCronService.new(store: store, enabled: true)
