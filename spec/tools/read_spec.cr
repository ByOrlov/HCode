...
describe H2code::Tools::Read do
  ...
  it "reads a small file with N\\tcontent line format" do
    path = "/tmp/h2code-test-read-basic.txt"
    File.write(path, "alpha\nbeta\ngamma\n")

    read = H2code::Tools::Read.new("/tmp")
    result = read.execute(JSON.parse(%({"path":"h2code-test-read-basic.txt"})))
  ...
  it "still accepts legacy filePath/offset/limit args" do
    path = "/tmp/h2code-test-read-legacy.txt"
    File.write(path, "one\ntwo\nthree\nfour\n")

    read = H2code::Tools::Read.new("/tmp")
    result = read.execute(JSON.parse(%({"filePath":"h2code-test-read-legacy.txt","offset":2,"limit":2})))
    result.is_error?.should be_false
  ...
  it "errors when file does not exist" do
    read = H2code::Tools::Read.new("/tmp")
    result = read.execute(JSON.parse(%({"path":"h2code-test-read-missing.txt"})))
    result.is_error?.should be_true
    result.content.should contain("does not exist")
  end

  it "errors when path is a directory" do
    dir = "/tmp/h2code-test-read-dir"
    Dir.mkdir_p(dir)

    read = H2code::Tools::Read.new("/tmp")
    result = read.execute(JSON.parse(%({"path":"h2code-test-read-dir"})))
    result.is_error?.should be_true
    result.content.should contain("not a file")
  end

  it "blocks sensitive files via PathAccess (.env)" do
    path = "/tmp/h2code-test-read-env/.env"
    Dir.mkdir_p("/tmp/h2code-test-read-env")
    File.write(path, "SECRET=shhh\n")

    read = H2code::Tools::Read.new("/tmp/h2code-test-read-env")
  ...
  it "allows .env.example (sensitive-file exemption)" do
    path = "/tmp/h2code-test-read-envex/.env.example"
    Dir.mkdir_p("/tmp/h2code-test-read-envex")
    File.write(path, "SECRET=placeholder\n")

    read = H2code::Tools::Read.new("/tmp/h2code-test-read-envex")
  ...
  it "rejects non-UTF-8 bytes" do
    ...
    read = H2code::Tools::Read.new("/tmp")
    result = read.execute(JSON.parse(%({"path":"h2code-test-read-badutf8.bin"})))
    result.is_error?.should be_true
    result.content.should contain("not readable as UTF-8 text")
  end

  it "respects line_offset and n_lines" do
    path = "/tmp/h2code-test-read-window.txt"
    File.write(path, (1..10).join('\n') + '\n')

    read = H2code::Tools::Read.new("/tmp")
    result = read.execute(JSON.parse(%({"path":"h2code-test-read-window.txt","line_offset":4,"n_lines":3})))
    result.is_error?.should be_false
  ...
  it "reads from the end of file with a negative line_offset" do
    ...
    read = H2code::Tools::Read.new("/tmp")
    result = read.execute(JSON.parse(%({"path":"h2code-test-read-tail.txt","line_offset":-3})))
    result.is_error?.should be_false
  ...
  it "folds pure CRLF files to LF for display" do
    path = "/tmp/h2code-test-read-crlf.txt"
    File.write(path, "alpha\r\nbeta\r\n")

    read = H2code::Tools::Read.new("/tmp")
    result = read.execute(JSON.parse(%({"path":"h2code-test-read-crlf.txt"})))
    result.is_error?.should be_false
    result.content.should contain("1\talpha")
    result.content.should contain("2\tbeta")
  ...
  it "renders mixed line endings with visible \\r" do
    path = "/tmp/h2code-test-read-mixed.txt"
    File.write(path, "alpha\r\nbeta\rgamma\n")

    read = H2code::Tools::Read.new("/tmp")
    result = read.execute(JSON.parse(%({"path":"h2code-test-read-mixed.txt"})))
    result.is_error?.should be_false
  ...
  it "truncates lines longer than MAX_LINE_LENGTH" do
    long = "x" * (H2code::Tools::Read::MAX_LINE_LENGTH + 100)
    path = "/tmp/h2code-test-read-long.txt"
    File.write(path, long + '\n')

    read = H2code::Tools::Read.new("/tmp")
    result = read.execute(JSON.parse(%({"path":"h2code-test-read-long.txt"})))
    result.is_error?.should be_false
  ...
  it "stops at the MAX_BYTES budget" do
    ...
    line = "y" * 199
    path = "/tmp/h2code-test-read-bytes.txt"
    File.write(path, (1..600).map { line }.join('\n') + '\n')

    result = read.execute(JSON.parse(%({"path":"h2code-test-read-bytes.txt"})))
    result.is_error?.should be_false
    result.content.should contain("Max #{H2code::Tools::Read::MAX_BYTES} bytes reached.")
  end

  it "caps n_lines at MAX_LINES" do
    path = "/tmp/h2code-test-read-cap.txt"
    File.write(path, (1..3000).join('\n') + '\n')

    read = H2code::Tools::Read.new("/tmp")
    result = read.execute(JSON.parse(%({"path":"h2code-test-read-cap.txt","n_lines":5000})))
    result.is_error?.should be_false
    result.content.should contain("Max #{H2code::Tools::Read::MAX_LINES} lines reached.")
    result.content.should contain("Total lines in file: 3000.")
  end

  it "reports end of file when fewer lines are available" do
    path = "/tmp/h2code-test-read-eof.txt"
    File.write(path, "only\n")

    read = H2code::Tools::Read.new("/tmp")
    result = read.execute(JSON.parse(%({"path":"h2code-test-read-eof.txt","n_lines":50})))
    result.is_error?.should be_false
    result.content.should contain("End of file reached.")
  end

  it "treats an empty file as zero lines" do
    path = "/tmp/h2code-test-read-empty.txt"
    File.write(path, "")

    read = H2code::Tools::Read.new("/tmp")
    result = read.execute(JSON.parse(%({"path":"h2code-test-read-empty.txt"})))
    result.is_error?.should be_false
