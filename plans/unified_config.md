...
|---|---|
| `H2CODE_DEBUG` | loop/agent.cr:307,383 · h2code.cr:650 · tools/cron.cr:301 · llm/openai_chat_provider.cr:505 |
| `H2CODE_DISABLE_CRON` | h2code.cr:811,885,908 · tools/cron.cr:841 |
| `H2CODE_CRON_NO_STALE` | tools/cron.cr:222 |
| `H2CODE_SUBAGENT_TIMEOUT_MS` | loop/subagent_agent_runner.cr:299 · loop/subagent_swarm_runner.cr:39 |
| `H2CODE_EXPERIMENTAL_FLAG` | tui/command_controller.cr:381 |
| `H2CODE_MOCK_SCRIPT` | llm/mock_provider.cr:399 |
...
Примечание: `H2CODE_HOME`, `HOME`, `HTTP_PROXY` и провайдерные ключи — это
...
if v = ENV["H2CODE_SUBAGENT_TIMEOUT_MS"]?.try(&.to_i?); v.try { |x| config.subagent_timeout_ms = x if x >= 1 } end
...
Сейчас `config` уже есть в `h2code.cr` (локальная переменная в `run` и `run_headless`).
...
1. **`Loop::Agent`** — добавить `property? debug : Bool = false` (или
   ...
   В `h2code.cr`, при создании `agent`, выставить `agent.debug = config.debug`.

2. **`LLM::Provider` / `MockProvider`** — `H2CODE_DEBUG` в
...
3. **`LiveCronService`** — в трёх местах `h2code.cr` (811/885/908) заменить
   `!ENV.has_key?("H2CODE_DISABLE_CRON")` на `config.cron_enabled`. В
   `tools/cron.cr:841` (сообщение об ошибке) и `:222` (`H2CODE_CRON_NO_STALE`)
...
4. **`SubagentAgentRunner` / `SubagentSwarmRunner`** — в `timeout_ms`
   заменить `ENV["H2CODE_SUBAGENT_TIMEOUT_MS"]?` на значение из конфига.
   Runners строятся в `wire_subagent_runners(..., config)` (h2code.cr:517) —
   туда и передать `config.subagent_timeout_ms` (или целиком поле).

5. **`TUI::CommandController`** (`H2CODE_EXPERIMENTAL_FLAG`) и
   **`TUI::InputController`** (`EDITOR/VISUAL/TMPDIR`) — App уже держит
   `app_config` (h2code.cr:702). Контроллеры берут значения из
   `app.app_config.experimental_flag` / `.editor` / `.tmp_dir`.

6. **`Tools::Bash`** (`GIT_TERMINAL_PROMPT`, `SHELL`) — Bash уже создаётся
   как объект в h2code.cr:802. Добавить в конструктор (или property) значения
...
7. **`Prompt::SystemPrompt`** (`SHELL`, system_prompt.cr:189) — передать
   `config.shell` в `SystemPrompt.build` (он уже вызывается из h2code.cr).

## Шаг 4. Удалить прямые `ENV[...]` в коде потребителей

После Шага 3 каждое чтение из таблицы выше заменяется на поле `config` /
`@debug` / property. Физически удалить `ENV["H2CODE_DEBUG"]?` и т.д. из:
loop/agent.cr, h2code.cr, tools/cron.cr, llm/openai_chat_provider.cr,
...
(`HOME`, `H2CODE_HOME`, провайдерные секреты, `HTTP_PROXY`).

## Шаг 5. Тесты и проверка

- В spec/config/ добавить кейсы: `Config.load` с выставленным ENV даёт корректные
  поля (`debug == true`, `cron_enabled == false` при `H2CODE_DISABLE_CRON`, и т.д.).
...
- Ручная smoke-проверка: `H2CODE_DEBUG=1` всё ещё включает `[debug]`-логи;
  `H2CODE_DISABLE_CRON=1` глушит cron — поведение не меняется, меняется только
  место чтения.
...
- Не выносим `HOME`/`H2CODE_HOME`/`HTTP_PROXY` и провайдерные секреты в новые
