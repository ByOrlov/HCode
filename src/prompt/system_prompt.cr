module Hcode
  module Prompt
    class SystemPrompt
      ...
      - For a code refactoring, you typically need to update all the places that call the code you are refactoring if the interface changes. DO NOT change any existing logic especially in tests, focus only on fixing errors caused by the interface changes.
      ...
      DO NOT run `git commit`, `git push`, `git reset`, `git rebase` and/or do any other git mutations unless explicitly asked to do so. Ask for confirmation each time when you need to git mutations, even if the user has confirmed in earlier conversations.
      ...
      - Avoid installing or deleting anything to/from outside of the current working directory. If you have to do so, ask for the user's confirmation.
      ...
      - When you have evidence the user is wrong, say so and show the evidence — agreeing to be agreeable wastes their time and can break their code. Defer once they have decided; until then, an honest objection is the helpful answer.
      ...
      private def self.directory_listing(dir : String, levels : Int32 = 2) : String
        return "(empty)" unless Dir.exists?(dir)

        lines = [] of String
        collect_listing(dir, 0, levels, lines)
      ...
      private def self.collect_listing(dir : String, level : Int32, max_levels : Int32,
        ...
        entries.reject! { |e| e.starts_with?('.') } if level == 0
        entries.reject! { |e| IGNORE_DIRS.includes?(e) }
        ...
        entries.each do |entry|
          ...
          if Dir.exists?(path) && !File.symlink?(path)
            lines << "#{indent}#{entry}/"
            collect_listing(path, level + 1, max_levels, lines) if level < max_levels - 1
