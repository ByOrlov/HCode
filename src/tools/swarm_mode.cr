module H2code
  module Tools
    ...
    # In-memory swarm-mode flag, mirroring the JS `IAgentSwarmService`. The
    # `/swarm` slash command flips it; `Loop::Agent` injects the enter-reminder
    # each step while it is on and auto-exits `task`/`tool` triggers at turn
    # end (a `manual` toggle persists across turns until the user turns it off).
    class SwarmModeService
      @active : Bool = false
      @trigger : SwarmTrigger? = nil

      def active? : Bool
        @active
      end

      def trigger : SwarmTrigger?
        @trigger
      end

      def enter(trigger : SwarmTrigger = SwarmTrigger::Manual) : Nil
        @active = true
        @trigger = trigger
      end
    ...
    module SwarmMode
      class_property service : SwarmModeService? = nil
      ...
      # Ported verbatim from `packages/agent-core/src/agent/swarm/enter-reminder.md`.
      ENTER_REMINDER = <<-TEXT
        <system-reminder>
        ## Swarm Mode
      ...
      # Ported verbatim from `packages/agent-core/src/agent/swarm/exit-reminder.md`.
      EXIT_REMINDER = <<-TEXT
        <system-reminder>
        ## Swarm Mode Ended
