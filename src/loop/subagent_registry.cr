  ...
  module Loop
    ...
    class ProfileRegistry
      def self.build(profile_name : String, work_dir : String) : Tools::Registry
        ...
        registry.register(Tools::Bash.new(work_dir)) if allowed.includes?("Bash")
        registry.register(Tools::Read.new(work_dir)) if allowed.includes?("Read")
        registry.register(Tools::Write.new(work_dir)) if allowed.includes?("Write")
        registry.register(Tools::Edit.new(work_dir)) if allowed.includes?("Edit")
        registry.register(Tools::Glob.new(work_dir)) if allowed.includes?("Glob")
        registry.register(Tools::Grep.new(work_dir)) if allowed.includes?("Grep")

        # stateless tools.
        registry.register(Tools::TodoList.new) if allowed.includes?("TodoList")
        registry.register(Tools::AgentSwarm.new) if allowed.includes?("AgentSwarm")
        registry.register(Tools::Agent.new) if allowed.includes?("Agent")
        registry.register(Tools::AskUserQuestion.new) if allowed.includes?("AskUserQuestion")
        registry.register(Tools::FetchURL.new) if allowed.includes?("FetchURL")
        registry.register(Tools::WebSearch.new) if allowed.includes?("WebSearch")
        registry.register(Tools::Skill.new) if allowed.includes?("Skill")
        registry.register(Tools::EnterPlanMode.new) if allowed.includes?("EnterPlanMode")
        registry.register(Tools::ExitPlanMode.new) if allowed.includes?("ExitPlanMode")
        registry.register(Tools::CreateGoal.new) if allowed.includes?("CreateGoal")
        registry.register(Tools::GetGoal.new) if allowed.includes?("GetGoal")
        registry.register(Tools::UpdateGoal.new) if allowed.includes?("UpdateGoal")
        registry.register(Tools::SetGoalBudget.new) if allowed.includes?("SetGoalBudget")
        registry.register(Tools::TaskList.new) if allowed.includes?("TaskList")
        registry.register(Tools::TaskOutput.new) if allowed.includes?("TaskOutput")
        registry.register(Tools::TaskStop.new) if allowed.includes?("TaskStop")
        registry.register(Tools::CronCreate.new) if allowed.includes?("CronCreate")
        registry.register(Tools::CronList.new) if allowed.includes?("CronList")
        registry.register(Tools::CronDelete.new) if allowed.includes?("CronDelete")
        registry.register(Tools::ReadMediaFile.new) if allowed.includes?("ReadMediaFile")
